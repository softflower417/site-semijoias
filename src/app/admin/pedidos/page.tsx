          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              requests.map((r) => (
                <tr
                  key={r.id}
                  className={`hover:bg-gray-50 cursor-pointer ${r.status === "pending" ? "font-semibold" : ""}`}
                  onClick={() => {
                    setSelected(r);
                    if (r.status === "pending") updateStatus(r.id, "seen");
                  }}
                >
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`text-xs px-2 py-1 rounded-full font-normal ${statusColor[r.status]}`}
                    >
                  ...
                  <td className="px-5 py-4 text-right">
                    <button
                      ...
                      className="text-gray-400 hover:text-brand-burgundy p-1"
                    >
                      <Eye size={18} />
                    </button>
                    {r.status !== "done" && (
                      <button
                        ...
                        className="text-gray-400 hover:text-green-600 p-1 ml-1"
        ...
        >
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full ${statusColor[selected.status]}`}
              >
            ...
            <div className="flex gap-3">
              ...
              <a
                href={`https://wa.me/55${selected.whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(`Olá ${selected.name}! Vi sua solicitação na MAONA e vim te atender.`)}`}
                target="_blank"
