...
export default function EditarProduto({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchProduct() {
      ...
      const imgs = (data.product_images || []).sort(
    ...
    for (let i = 0; i < newImages.length; i++) {
      ...
      if (uploadData) {
        ...
        if (!urlData?.publicUrl) {
          console.error(
            "Não foi possível gerar URL pública para nova imagem:",
            uploadData.path,
          );
          continue;
        }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Link
          href="/admin/produtos"
          className="text-gray-400 hover:text-brand-burgundy"
        >
          <ArrowLeft size={24} />
        </Link>
      ...
      >
        ...
        <section>
          ...
          <p className="text-xs text-gray-400 mb-4">
            A primeira foto é a capa. Você pode adicionar novas ou remover
            existentes.
          </p>
          <div className="flex flex-wrap gap-4">
            ...
            {newImages.map((img) => (
              <div
                key={img.tempId}
                className="relative w-28 h-28 border border-blue-200 rounded overflow-hidden group"
              >
                <img
                  src={img.preview}
                  alt=""
                  className="w-full h-full object-cover"
                />
                <span className="absolute bottom-0 left-0 right-0 text-center text-[10px] bg-blue-500 text-white py-0.5">
                  NOVA
                </span>
                <button
                  type="button"
                  onClick={() =>
                    setNewImages((prev) =>
                      prev.filter((n) => n.tempId !== img.tempId),
                    )
                  }
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold outline-none"
              >
                {["Brincos", "Colares", "Pulseiras", "Braceletes", "Anéis"].map(
                  (c) => (
        ...
        <section>
          ...
          <div className="flex gap-4">
            {[
              { v: "active", l: "Ativo (visível)" },
              { v: "inactive", l: "Inativo (oculto)" },
            ].map((opt) => (
              <label
                key={opt.v}
