  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/admin/produtos" className="text-gray-400 hover:text-brand-burgundy transition-colors">
          <ArrowLeft size={24} />
        </Link>
        <h1 className="text-2xl font-serif text-brand-burgundy">Novo Produto</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white p-8 rounded-lg shadow border border-gray-100">
        ...
        <section>
          ...
          <p className="text-xs text-gray-400 mb-4">A primeira foto será a capa. Adicione quantas quiser.</p>
          ...
          <div className="flex flex-wrap gap-4">
            {images.map((img, index) => (
              <div key={img.id} className="relative w-28 h-28 border border-gray-200 rounded overflow-hidden group">
                ...
                {index === 0 && (
                  <span className="absolute bottom-0 left-0 right-0 text-center text-[10px] bg-brand-gold text-white py-0.5">CAPA</span>
                )}
                <button
                  type="button"
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold focus:border-brand-gold outline-none" />
            </div>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-3 gap-4 items-end">
            <div>
              ...
              <input required type="text" placeholder="199.90" name="price" value={formData.price} onChange={handleChange} className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold outline-none" />
            </div>
            <div className="flex items-center h-[42px] pb-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_on_sale" checked={formData.is_on_sale} onChange={handleChange} className="w-4 h-4 text-brand-gold focus:ring-brand-gold rounded" />
            ...
            {formData.is_on_sale && (
              <div>
                ...
                <input required type="text" placeholder="149.90" name="sale_price" value={formData.sale_price} onChange={handleChange} className="w-full px-4 py-2 border border-red-300 rounded focus:ring-1 focus:ring-red-500 outline-none" />
              </div>
            )}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" placeholder="Ex: Prata 925, Liga Metálica" name="material" value={formData.material} onChange={handleChange} className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Banho</label>
              <input type="text" placeholder="Ex: Ouro 18k 10 milésimos" name="plating" value={formData.plating} onChange={handleChange} className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Medidas</label>
              <input type="text" placeholder="Ex: 40cm + 5cm extensor" name="measurements" value={formData.measurements} onChange={handleChange} className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Garantia</label>
              <input type="text" name="warranty" value={formData.warranty} onChange={handleChange} className="w-full px-4 py-2 border rounded focus:ring-1 focus:ring-brand-gold outline-none" />
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="status" value="active" checked={formData.status === 'active'} onChange={handleChange} className="text-brand-gold" />
              <span className="text-sm text-gray-700">Ativo (visível no site)</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="status" value="inactive" checked={formData.status === 'inactive'} onChange={handleChange} className="text-brand-gold" />
        ...
        <div className="flex justify-end pt-4">
          <button type="submit" disabled={loading} className="bg-brand-burgundy text-white px-8 py-3 rounded uppercase tracking-widest text-sm hover:bg-brand-burgundy/90 transition-colors flex items-center gap-2 disabled:opacity-50">
            <Save size={18} />
